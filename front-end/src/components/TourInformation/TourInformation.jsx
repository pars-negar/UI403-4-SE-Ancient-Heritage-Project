...
const TourInformation = ({ tour }) => {
    ...
    useEffect(() => {
        if (tour && tour.daily_schedules) {
            setTourPlan(tour.daily_schedules);
        }
    }, [tour]);

    if (!tour || Object.keys(tour).length === 0) {
        return <div className="text-center mt-10">اطلاعات تور در دسترس نیست.</div>;
    }
        ...
        <div className="bg-white w-[80%] rounded-[30px]">
            <>
                ...
                >
                    ...
                    {/* بخش راهنمایان تور */}
                    {tour.tour_guides_info && Array.isArray(tour.tour_guides_info) && tour.tour_guides_info.length > 0 && (
                        <div className="mb-12">
